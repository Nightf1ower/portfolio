(() => {
  ...
  function card(item, list, index, caption, hoverItems = []) {
    ...
    if (hoverItems.length) {
      ...
      let hoverIndex = 0;
      const showHover = () => {
        hover.src = q(hoverItems[hoverIndex].url);
        hover.alt = `${img.alt} hover`;
        hoverIndex = (hoverIndex + 1) % hoverItems.length;
      };
      showHover();
      hover.loading = 'lazy';
      media.append(hover);
      button.addEventListener('mouseenter', () => {
        ...
        showHover();
        clearInterval(timer);
        timer = setInterval(showHover, 700);
      });
  ...
  async function open() {
    ...
    inner.append(makeSection('PRINTS', printGroups(data.prints).length, renderPrints(data.prints), 'Наведи на принт: сначала версия, потом товар, потом вариант на товаре.'));
