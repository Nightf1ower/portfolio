(() => {
  ...
  style.textContent = `
    .cr-modal .cr-card-label { display: none !important; }
    ...
    .cr-subgroup-vinyl-grid .cr-card { width: 100% !important; }
    .cr-section-merch-clean .cr-grid { align-items: start !important; }
    .cr-section-album-curated .cr-subgroup + .cr-subgroup { margin-top: clamp(4rem, 8vw, 7rem) !important; }

    @media (max-width: 900px) {
      .cr-subgroup-vinyl-grid .cr-grid,
      .cr-section-merch-clean .cr-grid { grid-template-columns: repeat(2, minmax(0, 1fr)) !important; }
    }

    @media (max-width: 600px) {
      .cr-subgroup-vinyl-grid .cr-grid,
      .cr-section-merch-clean .cr-grid { grid-template-columns: 1fr !important; }
    }
  `;
  ...
  function apply(modal = document.querySelector('.cr-modal')) {
    ...
    if (albumSection) {
      ...
      [...(subgroups[1]?.querySelectorAll('.cr-card') || [])].slice(6).forEach((card) => card.remove());
    }

    const merchSection = findSection(modal, 'MERCH');
    if (merchSection) {
      merchSection.classList.add('cr-section-merch-clean');
      [...merchSection.querySelectorAll('.cr-card')][0]?.remove();
    }
